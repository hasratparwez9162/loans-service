...
public class LoansController {
        ...
        public ResponseEntity<Loan> updateLoanStatus(@PathVariable Long loanId, @RequestParam String status) throws JsonProcessingException {
            Loan updatedLoan = loanService.updateLoanStatus(loanId, status);
            // Send message after update loan status
            loanEventProducer.sendLoanStatusUpdatedMessage(updatedLoan);
            return new ResponseEntity<>(updatedLoan, HttpStatus.OK);
        }

        @PutMapping("/repay/{loanId}")
        public ResponseEntity<Loan> repayLoan(@PathVariable Long loanId, @RequestParam BigDecimal amount) throws JsonProcessingException {
